...
foreach ($lista_array_geral as $v) {
	...
	//TEMPO	BRUTO
	array_push($lista_geral[$i], '<b>'.substr($v['tempo'],0,10)."</b>");

	//PENAIS - BONUS
	$str_penais_bonus = '<div style="color:red">'.substr($v['penalidade'],0,9)."</div>";
	$str_penais_bonus .= '<div style="color:blue"><br>'.substr($v['bonus'],0,9)."</div>";
	array_push($lista_geral[$i], $str_penais_bonus);

	//TEMPO TOTAL - DIF. LIDER
	$str_tempo_total = '<div style="font-size:14px"><b>'.substr($v['total'],0,9)."</b></div>";
	$str_tempo_total .='<br>'.substr($v['diferenca_lider'],0,10);
