...
function printHeader($txt_pag, $txt_timestamp, $iFIM) {
	...
	$header .= sprintf("%s\n","     <img src=\"imagens/sertoes2016.jpg\"width=120 border=0 valign=\"center\" align=\"absmiddle\"/>");
	$header .= sprintf("%s\n","    </td>");
	
	
	$header .= sprintf("%s\n","    <td align=\"right\" valign=\"top\">");
	$header .= sprintf("%s\n","     <img src=\"imagens/patro3_sertoes_2016.jpg\" width=800 border=0 align=\"absmiddle\"/>&nbsp;");
	//$header .= sprintf("%s\n","     <img src=\"imagens/patr4.jpg\" border=0 align=\"absmiddle\"/>");
	$header .= sprintf("%s\n","    </td>");		
	$header .= sprintf("%s\n","</tr>");	
	$header .= sprintf("%s\n","</table>");	
	$header .= sprintf("%s\n","  <table border=\"0\" width=\"100%\" height=\"80\" cellpadding=5 cellspacing=0>");	
	$header .= sprintf("%s\n","<tr>");					
...
function geraFooter($iCat, $iMod, $strMod) {
	...
	$footer .= sprintf("%s\n","<td align=\"left\">");
	//$footer .= sprintf("%s\n","     <img src=\"imagens/patro_sertoes_2016.jpg\"width=250 border=0 align=\"absmiddle\"/>&nbsp;");
	//$footer .= sprintf("%s\n","     <img src=\"imagens/cobreq.png\"width=100 border=0 align=\"absmiddle\"/>&nbsp;");
	//$footer .= sprintf("%s\n","     <img src=\"imagens/pm-PontaGrossa.png\"width=80 border=0 align=\"absmiddle\"/>&nbsp;");
	//$footer .= sprintf("%s\n","     <img src=\"imagens/fumtur.png\"width=100 border=0 align=\"absmiddle\"/>");
	
	$footer .= sprintf("%s\n"," </td><td align=\"right\">");	$footer .= ($_REQUEST['db']=='1' || $_REQUEST['db']=='local1') ? 					sprintf("%s   ","<img src=\"imagens/ass_dunas_cba_chrono.png\" width=600 border=0 align=\"absmiddle\"/>") : 					sprintf("%s   ","<img src=\"imagens/ass_dunas_cbm_chrono.png\" width=600 border=0 align=\"absmiddle\"/>");	$footer .= sprintf("%s\n","</td></tr></table>");	return $footer;
}	
...
*
*/
function geraTxtPag($sPag, $sTrechos, $iTrecho) {	global $arr_ss;		$titulo = $_REQUEST['titulo'];	if (!$iTrecho){		$ult = sizeof($arr_ss)-1;		$iTrecho =$arr_ss[$ult];	}		$tre = criaArray ("SELECT * FROM t02_trecho WHERE c02_codigo=".$iTrecho);	$dist_esp_tot = $tre[0]["c02_distancia"] + $tre[0]["c02_desl_ini"] + $tre[0]["c02_desl_fin"];	$txt_Pag = ($sPag == "geral") ? "ACUMULADO AT&Eacute; " : "";		$txt_Pag .= $tre[0]["c02_nome"].": ".$tre[0]["c02_origem"]." - ".$tre[0]["c02_destino"]." (".$dist_esp_tot."km)";	if (strlen($titulo) > 0) $txt_Pag .= "<br />".$titulo;		return $txt_Pag;}
