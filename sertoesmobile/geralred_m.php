...
else if (strlen($int_id_ss) > 0) $numero_trecho = $int_id_ss;//if (isset($_REQUEST["trechos"])) $numero_trecho = null;
...
foreach ($lista_array as $v) {
	...
	if (strlen($v['modelo']) > 0) $tripulacao .= $v['modelo']."<br>";		//if (strlen($v['equipe']) > 0) $tripulacao .= $v['equipe']."<br>";
	$tripulacao .= '</div>';
	...
	//PENAIS - BONUS
	$str_penais_bonus = '<div style="color:red; font-size:20px">'.substr($v['penalidade'],0,8)."</div>";
	$str_penais_bonus .= '<div style="color:blue; font-size:20px"><br>'.substr($v['bonus'],0,8)."</div>";
	array_push($lista[$i], $str_penais_bonus);

	//TEMPO TOTAL - DIF. LIDER
	$str_tempo_total = '<div style="font-size:14px"><b>'.substr($v['total'],0,8)."</b></div>";	$str_tempo_total .='<br>'.substr($v['diferenca_lider'],0,8);	$str_tempo_total .='<br>'.substr($v['diferenca_anterior'],0,8);
...
array_push($campos_header_ss,'<div style="color:red;font-size:20px">Penal</div><div style="color:blue;font-size:20px"><br>Bonus</div>');
array_push($campos_header_ss,'TOTAL<div style="font-size:20px"><br>Dif. Lider</div><div style="font-size:20px">Dif.Anterior</div>');
