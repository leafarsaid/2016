...
$xml_src = $strBaseURL."/intermediariasXML.php?".$_SERVER['QUERY_STRING'];
...
foreach ($lista_array_ss as $v) {
	...
	if (strlen($v['equipe']) > 0) $tripulacao .= $v['equipe']."<br>";
	...
	//TEMPO	INTERMEDIARIA 1
	if ($_REQUEST["intermediarias"]>=1) array_push($lista_ss[$i], '<b>'.($v['tempo1'])."</b>");
	
	//TEMPO	INTERMEDIARIA 2
	if ($_REQUEST["intermediarias"]>=2) array_push($lista_ss[$i], '<b>'.($v['tempo2'])."</b>");
	
	//TEMPO	INTERMEDIARIA 3
	if ($_REQUEST["intermediarias"]>=3) array_push($lista_ss[$i], '<b>'.($v['tempo3'])."</b>");
	
	//TEMPO	INTERMEDIARIA 4
	if ($_REQUEST["intermediarias"]>=4) array_push($lista_ss[$i], '<b>'.($v['tempo4'])."</b>");

	//TEMPO	BRUTO
	array_push($lista_ss[$i], '<b>'.substr($v['tempo'],0,10)."</b>");
	...
	//TEMPO - DIF. LIDER
	$str_tempo_total = '<div style="font-size:14px"><b>'.substr($v['total'],0,10)."</b></div>";
	$str_tempo_total .='<br>'.substr($v['diferenca_lider'],0,10);
	$str_tempo_total .='<br>'.substr($v['diferenca_anterior'],0,10);
	array_push($lista_ss[$i], $str_tempo_total);
	
	
	//VELOCIDADE MEDIA
	array_push($lista_ss[$i], round($v['vel_media'],2));
...
if ($_REQUEST["intermediarias"]>=1) array_push($campos_header_ss,"INT.1");
if ($_REQUEST["intermediarias"]>=2) array_push($campos_header_ss,"INT.2");
if ($_REQUEST["intermediarias"]>=3) array_push($campos_header_ss,"INT.3");
if ($_REQUEST["intermediarias"]>=4) array_push($campos_header_ss,"INT.4");
...
array_push($campos_header_ss,'VEL. M&Eacute;DIA');
